//
//  AWFMKStormCellAnnotationView.h
//  AerisMap
//
//  Created by Nicholas Shipes on 5/6/14.
//  Copyright (c) 2014 AerisWeather, LLC. All rights reserved.
//

#import <Foundation/Foundation.h>
#import <AerisMapKit/AWFMKAnnotationView.h>

NS_ASSUME_NONNULL_BEGIN

/**
 An `AWFMKStormCellAnnotationView` object is an `AWFMKAnnotationView` subclass to represent storm cell-related annotations on a weather map
 that is configured to use the `AWFWeatherMapTypeApple` type.
 */
@interface AWFMKStormCellAnnotationView : AWFMKAnnotationView

@end

NS_ASSUME_NONNULL_END
